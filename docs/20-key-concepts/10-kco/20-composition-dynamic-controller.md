...
The composition-dynamic-controller is an operator instantiated by the [Core Provider](10-core-provider.md) to manage the Custom Resources whose Custom Resource Definition is generated by the Core Provider.

This page evolves the CDC documentation for Docusaurus and keeps the explanation aligned with the rest of the KCO concepts.
...
| `OTEL_ENABLED`                        | Enables OTLP metrics export for runtime and operation telemetry | `false`      | Use `--metrics-enabled` flag |
