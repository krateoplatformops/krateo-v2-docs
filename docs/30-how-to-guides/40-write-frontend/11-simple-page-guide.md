...
To create a `Button` widget, you need to define it as a Kubernetes resource using a YAML file that follows the widget schema described in the [Widgets API Reference](./widget-api-reference). Applying the resource will both create the widget and validate its configuration.
