...
```shell
export SERVICE_IP=$(kubectl get svc --namespace krateo-system krateo-frontend --template "{{"{{ range (index .status.loadBalancer.ingress 0) }}{{.}}{{ end }}"}}")
...
```shell
vcluster delete krateo-vcluster
```
